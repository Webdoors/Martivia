...
ORDER BY `t5`.`Id` ASC LIMIT 1) as "scoid"    FROM `scoring` as t1 LEFT JOIN merchants as t6 ON(t1.muser=t6.id) WHERE t1.id>0 AND t6.name LIKE '."'".'lightspeed%'."'".'  '.$STA.' ORDER BY t1.Id DESC LIMIT 30 OFFSET '.$fr.'');
...
ORDER BY `t5`.`Id` ASC LIMIT 1) as "scoid"    FROM `scoring` as t1 WHERE t1.id>0 AND (t1.muser=14 OR t1.muser=16 OR t1.muser=17) '.$STA.' ORDER BY t1.Id DESC ');
...
ORDER BY `t3`.`Id` ASC LIMIT 1) as "scoid"    FROM `scoring` as t1 LEFT JOIN merchants as t4 ON(t1.muser=t4.id) WHERE t1.id>0 AND t4.name LIKE '."'".'lightspeed%'."'".' '.$STA.'  ORDER BY t1.userid DESC');
?>
<ul class="pagination">
<?php
...
?>
	<li class="<?=($ACP==$i?"active":"")?>"><a href="?page=reports&id=2&<?=$FPO?>&p=<?=$i?>"><?=$i?></a>
	</li>
<?	
}
?>	
