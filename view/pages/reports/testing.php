...
ORDER BY `t5`.`Id` ASC LIMIT 1) as "scoid"    FROM `scoring` as t1 WHERE t1.Id>0 AND (t1.muser=14 OR t1.muser=16 OR t1.muser=17) '.$STA.' ORDER BY t1.Id DESC LIMIT 30 OFFSET '.$fr.'');
...
ORDER BY `t3`.`Id` ASC LIMIT 1) as "scoid"    FROM `scoring` as t1 WHERE t1.id>0 AND (t1.muser=14 OR t1.muser=16 OR t1.muser=17) '.$STA.'  ORDER BY t1.userid DESC');
?>
<ul class="pagination">
<?php
