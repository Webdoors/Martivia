<?php
 $seo=mysqli_query($con,"SELECT t1.* , t1.column_value AS description FROM seo AS t1 WHERE t1.page_name='contact' AND t1.table_column='description' ");
 if(mysqli_num_rows($seo)==0)
 {
	 mysqli_query($con,"INSERT INTO seo SET page_name='contact', table_column='description' "); 
	 ?>
...
<div class="input-append row">
				  ...
				  <button class="btn iframe-btn btn-outline-primary insk" t="" pg='contact' n="keyword" d="0">Select</button>
				</div>
...
<br/>
  <div class='ajaxkeywords w-100' style='overflow:hidden'>
       <?php
	          $kw=mysqli_query($con,"SELECT * FROM seo WHERE table_column='keyword'  AND page_name='contact' ");
			  while($rkw=mysqli_fetch_assoc($kw))
			  {
				  ?>
				  <div class='dlkw' d='<?=$rkw['id'] ?>' t='' pg='contact' pid='0' >
				     <p> <?=$rkw['keywords'] ?> <p>
				  </div>
      		  <?php		  
			  }
