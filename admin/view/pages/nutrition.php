...
if($a!=""){
	...
	(SELECT column_value FROM seo WHERE table_id=t1.id AND   page_name='nutrition'  AND table_name='articles' AND table_column='description' AND t1.id='".$a."') AS description
	FROM articles as t1	WHERE t1.id='".$a."' ");
	$r1=mysqli_fetch_array($q1);
}else{

	$ina=mysqli_query($con,"INSERT INTO articles (title,text,img,slug,date,active,nutritions) VALUES ('','','','','$T',1,1) ");
...
<script>location.href="?page=nutrition&id=<?=$a ?>"</script>
<?php
	}
    $desc=mysqli_query($con,"SELECT * FROM seo WHERE page_name='nutrition' AND  table_name='articles' AND table_id='$a' AND table_column='description' ");
	if(mysqli_num_rows($desc)==0)
	{
		mysqli_query($con,"INSERT INTO seo SET page_name='nutrition',  table_name='articles', table_id='$a', table_column='description'");
...
$a=mysqli_real_escape_string($con,$_GET["id"]);
$q1=mysqli_query($con,"SELECT t1.*,
	(SELECT column_value FROM seo WHERE table_id=t1.id AND  page_name='nutrition' AND table_name='articles' AND table_column='description') AS description
	FROM articles as t1	WHERE t1.id='".$a."' ");
$r1=mysqli_fetch_array($q1);
?>
<div role="tabpanel">
...
<br/>
   ...
   <div class="tab-content">
       <div class="col-sm-12 pad-no-right pad-no-xs NOP">
	   <label for="pdf">authors</label>
	   
			<select class="form-control auth" name="ACA">
			<option <?=$r1["author"]==0?"selected":""?> value='0' >author</option>
<?php
$q3=mysqli_query($con,"SELECT * FROM admins ORDER BY id DESC");
while($r3=mysqli_fetch_array($q3)){
?>
				<option <?=$r1["author"]==$r3["id"]?"selected":""?> value="<?=$r3["id"]?>"><?=$r3["name"] ?> - <?=$r3["lastname"] ?></option>
<?php
}
?>				
			</select>
...
<div class="input-append row">
				  ...
				  <button class="btn iframe-btn btn-primary insk" t="articles" pg='nutrition' n="keyword" d="<?=$r1['id'] ?>">Select</button>
				</div>
...
<br/>
  <div class='ajaxkeywords'>
       <?php
	          $kw=mysqli_query($con,"SELECT id, keywords FROM seo WHERE table_id='$a' AND table_column='keyword'  AND page_name='nutrition' AND table_name='articles' ");
			  while($rkw=mysqli_fetch_assoc($kw))
			  {
				  ?>
				  <div class='dlkw' d='<?=$rkw['id'] ?>' t='articles' tn='nutrition' pid='<?=$r1['id'] ?>' >
				     <p> <?=$rkw['keywords'] ?> <p>
				  </div>
      		  <?php		  
			  }
...
<br/>
    ...
    <div class="submit my-4">
        <button  class="btn btn-success SAVNUTR" d="<?=$a?>">დამახსოვრება</button>
    </div>
