...
LEFT JOIN categories as t2 ON(t1.category=t2.id)
	WHERE t1.id>0 AND t1.nutritions=0 $WSE ORDER BY t1.id DESC LIMIT ".$PA." OFFSET ".$fr."");	
	...
	<div class="col-sm-12">
	   <div class="row my-2">
		...
		<div class="row my-2">
			...
			<div class="col-sm-2">
				<a class="btn btn-success" href="?page=article" data-original-title="" title="">Training-ის დამატება</a>
			</div>
		</div>
	</div>
	<div class="col-sm-12">
...
?>
		  <tr>
			...
			<th><a href="https://ikaweightlifter.com/training/<?=$r1['slug'] ?>/<?=$r1["id"]?>"><button class="btn btn-outline-primary">ნახვა</button></a></th>
...
?>
	...
	<a href="?page=articles&p=1&cid=<?=$cid!=""?$cid:""?>" class="PG USR">«</a>
	<a href="?page=articles&p=<?=$ACP!=1?($ACP-1):$ACP?>&cid=<?=$cid!=""?$cid:""?>" class="PG USR">‹</a>
	<?php
	for($i=1;$i<=ceil(mysqli_num_rows($q3)/$PA);$i++){
		if(($ACP-5)<=$i&&($ACP+5)>=$i){
	?>
	<a href="?page=articles&p=<?=$i?>&cid=<?=$cid!=""?$cid:""?>" class="PG <?=($ACP==$i?"ACP":"")?> USR"><?=$i?></a>
	<?php }
	}
	?>
	<a href="?page=articles&p=<?=$ACP!=ceil(mysqli_num_rows($q3)/$PA)?($ACP+1):$ACP?>&cid=<?=$cid!=""?$cid:""?>" class="PG USR">›</a>
	<a href="?page=articles&p=<?=ceil(mysqli_num_rows($q3)/$PA);?>&cid=<?=$cid!=""?$cid:""?>" class="PG USR">» <?=ceil(mysqli_num_rows($q3)/$PA);?></a>
	</div>
</div>
</div>
<br>