<?php
 $seo=mysqli_query($con,"SELECT t1.* , t1.column_value AS description, (SELECT column_value FROM langs WHERE table_name='seohome' AND table_column='description' AND short_name='en' ) AS endescription,
(SELECT column_value FROM langs WHERE table_name='seohome' AND table_column='description' AND short_name='ru' ) AS rudescription
 FROM seo AS t1 WHERE t1.page_name='home' AND t1.page_column='description' ");
 if(mysqli_num_rows($seo)==0)
 {
	 mysqli_query($con,"INSERT INTO seo SET page_name='home', page_column='description' "); 
	 ?>
  ...
  $ln=mysqli_query($con,"SELECT * FROM langs WHERE table_name='seohome' AND table_column='description' AND short_name='en' ");
 if(mysqli_num_rows($ln)==0)
 {
	 mysqli_query($con,"INSERT INTO langs SET table_name='seohome' , name='english', table_column='description', short_name='en'  ");
 }
  $ln1=mysqli_query($con,"SELECT * FROM langs WHERE table_name='seohome' AND table_column='description' AND short_name='ru' ");
 if(mysqli_num_rows($ln1)==0)
 {
	 mysqli_query($con,"INSERT INTO langs SET table_name='seohome' , name='russian', table_column='description', short_name='ru'  ");
 }
...
<div class="input-append row">
				  ...
				  <button class="btn iframe-btn btn-outline-primary insk" t="home" n="keyword" d="0">Select</button>
				</div>
...
<br/>
  <div class='ajaxkeywords w-100'>
       <?php
	          $kw=mysqli_query($con,"SELECT * FROM seo WHERE page_column='keyword'  AND page_name='home' ");
			  while($rkw=mysqli_fetch_assoc($kw))
			  {
				  ?>
				  <div class='dlkw' d='<?=$rkw['id'] ?>' t='home' pid='0' >
				     <p> <?=$rkw['keywords'] ?> <p>
				  </div>
      		  <?php		  
			  }
